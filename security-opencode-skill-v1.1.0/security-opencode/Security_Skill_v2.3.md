# Ultimate Security Skill v2.3: Defense-Grade SaaS Architect
## CMMC 2.0 · NIST CSF 2.0 · NIST SP 800-171/800-53/800-207 · OWASP Top 10:2025 · OWASP ASVS 5.0 · CISA Secure by Design · EO 14028 SBOM · deepsec AI Scanner · 2026 Penetration Patterns

---

## 1. Skill Directive & Activation

When utilizing GODMYTHOSV7 (or any descendant skill) inside Kilo Code, OpenClaw, Claude Code, or any AI-assisted development tool to generate or architect SaaS platforms, you **must** concurrently activate this Ultimate Security Skill. This skill mandates that all SaaS architecture, code, and infrastructure adhere to:
...
**Activation trigger**: Append the following constraint to all GODMYTHOSV7 outputs when building SaaS:

> "Evaluate this code/architecture against the Ultimate Security Skill v2.3. Does it enforce Zero Trust, Least Privilege, FIPS-validated encryption, OWASP Top 10:2025 mitigations, supply chain integrity, SBOM generation, NIST CSF 2.0 Govern-function alignment, the 9-phase security-scanner pipeline with deepsec AI deep review, BOLA ownership validation with detection logic, OAuth scope minimization, and AI credential protection with automated verification? If not, rewrite to enforce compliance."
...
### 12.1 BOLA/IDOR Deep Dive — Mandatory Ownership Validation

BOLA (API1) is the #1 risk on the OWASP API Security Top 10 and the root cause of the Lovable platform breach (April 2026), where any free-tier account could read other users' source code, database credentials, and AI chat histories via five unauthenticated API calls.

**Every API endpoint that returns or mutates a resource MUST enforce ownership validation:**

```
// VULNERABLE — Lovable pattern: authenticates user but skips ownership check
GET /api/projects/{project_id}/messages
→ Verifies Firebase auth token ✓
→ Does NOT check project.owner_id == request.user_id ✗

// SECURE — Mandatory pattern for this skill
GET /api/projects/{project_id}/messages
→ Verifies auth token ✓
→ Verifies project.owner_id == request.user_id ✓  (or RLS policy)
→ Returns 404 (not 403) if ownership fails ✓     (prevents enumeration)
```

**BOLA Scanning Rules:**

| Rule ID | Detection Pattern | Detection Logic (Regex/Behavioral) | Severity | Phase |
|---|---|---|---|---|
| BOLA-001 | API endpoint returns data without verifying `user_id == resource.owner_id` | SAST: Identify API handlers that retrieve objects without a `WHERE` clause or conditional check verifying `request.user_id == resource.owner_id`. Semantic: deepsec `id_naming_violation` detector. | CRITICAL | 1, 9 |
| BOLA-002 | Database query uses client-supplied ID without WHERE clause on tenant/owner | Regex: `SELECT.*FROM.*WHERE.*id\s*=\s*\$` without subsequent `AND.*owner_id`. AST: Query builder calls missing tenant filter. | CRITICAL | 1, 9 |
| BOLA-003 | Sequential/predictable resource IDs (integers) exposed in API | Regex: Route params matching `/:\w+Id/` bound to integer type. DAST: Increment ID in response, check if new resource returned. | HIGH | 1, 6 |
| BOLA-004 | API returns 403 (instead of 404) on unauthorized access — enables enumeration | DAST: `nuclei` template sends cross-tenant request; flag if response is 403 instead of 404. | MEDIUM | 6 |
| BOLA-005 | Missing Row-Level Security (RLS) policies on multi-tenant tables | SQL audit: `SELECT tablename FROM pg_tables WHERE tablename NOT IN (SELECT tablename FROM pg_policies)` against schema. | CRITICAL | 1, 9 |
| BOLA-006 | RLS policy exists but is disabled or has `USING (true)` clause | SQL audit: `SELECT * FROM pg_policies WHERE qual = 'true'` or `ALTER TABLE ... DISABLE ROW LEVEL SECURITY`. | CRITICAL | 1, 9 |
| BOLA-007 | Sensitive data exposure in API response payload | Schema scan: Response JSON keys matching `password\|secret\|api_key\|token\|ssn\|credit_card` without masking/redaction. | HIGH | 6, 9 |

**Verification Commands:**

```bash
# Manual ownership logic check (run from repo root)
grep -rE "owner_id|user_id|tenant_id" ./internal/api/ ./internal/handlers/ ./pkg/

# Find API handlers missing ownership WHERE clause
grep -rn "SELECT.*FROM" ./internal/ | grep -v "owner_id\|tenant_id\|user_id"

# PostgreSQL RLS audit (run against database)
psql -c "SELECT schemaname, tablename FROM pg_tables
         WHERE schemaname = 'public'
         AND tablename NOT IN (SELECT tablename FROM pg_policies);"

# DAST: nuclei BOLA template (run against staging)
nuclei -t bola-idor.yaml -u https://staging.example.com/api/ -H "Authorization: Bearer $CROSS_TENANT_TOKEN"
```

**Implementation Requirements:**
- Use UUIDs (v4/v7), not sequential integers, for all resource identifiers.
- Enforce Row-Level Security (RLS) at the database layer (Supabase/PostgreSQL) as defense-in-depth — application-layer checks alone are insufficient if a regression disables them.
- Return 404 (not 403) for resources the user doesn't own — prevents ID enumeration.
- Automated BOLA testing in CI: `nuclei` templates or custom scripts that attempt cross-tenant resource access.

---

## 13. AI/SaaS-Specific Attack Vectors (2026 Penetration Patterns)

This section codifies attack vectors derived from real-world 2026 breaches targeting AI-native SaaS platforms. These patterns are not theoretical — they are documented, exploited, and must be defended against.

### 13.1 OAuth Scope Creep & Token Lateral Movement

**Source breach:** Vercel/Context.ai (April 2026). A Vercel employee granted an AI productivity tool "Allow All" OAuth permissions using their corporate Google Workspace account. When Context.ai was compromised via Lumma Stealer malware, the attacker pivoted through the OAuth token into Vercel's Google Workspace, then laterally into Vercel's internal systems, enumerating and exfiltrating environment variables.

**Attack chain (MITRE ATT&CK mapped):**
```
Context.ai employee infected (T1555 Credential Stores)
  → OAuth tokens stolen from Context.ai's Supabase (T1539 Steal Web Session Cookie)
    → Vercel employee's Google Workspace accessed (T1078.004 Cloud Accounts)
      → Lateral movement into Vercel internal systems (T1550 Use Alternate Auth)
        → Environment variables enumerated and decrypted (T1530 Data from Cloud Storage)
          → Data exfiltrated and offered for sale (T1567 Exfiltration)
```

**OAuth Governance Controls (MANDATORY for all SaaS):**

| Control | Requirement | Implementation |
|---|---|---|
| OA-001 | OAuth app approval gating | Admin approval required before any OAuth integration; no self-service grants |
| OA-002 | Scope minimization | Request minimum scopes per API call; never `Allow All` / `full access` |
| OA-003 | OAuth app inventory | Maintain live inventory of all OAuth grants across Google Workspace / Entra ID / Okta |
| OA-004 | Token lifecycle | OAuth tokens ≤90 day expiry; refresh tokens bound to device; revoke on employee offboarding |
| OA-005 | Shadow AI detection | Monitor for unauthorized AI tool OAuth grants; alert on new app authorizations |
| OA-006 | Sensitive env var encryption | All environment variables containing secrets MUST be marked "sensitive" (encrypted at rest) |
| OA-007 | Cross-boundary audit logging | Log all OAuth token usage across integrated services; feed to SIEM |
| OA-008 | Vendor notification SLA | Contractual ≤24hr notification from third-party vendors on detecting unauthorized access |

**Scanning rules for deepsec / semantic anomaly detection:**

| Rule ID | Detection Pattern | Detection Logic (Scope/Token Analysis) | Severity | Phase |
|---|---|---|---|---|
| OAUTH-001 | OAuth scope requests broader than API calls actually used | Trust graph: Map granted scopes to actual API calls; flag wildcards (`*`) or `all` instead of resource-level scopes (e.g., `read:user:101` vs `read:all`). | CRITICAL | 9 |
| OAUTH-002 | Long-lived OAuth tokens (>90 days) without rotation | Token audit: Query OAuth provider API for token age; flag tokens exceeding rotation threshold without revocation mechanism. | HIGH | 3, 9 |
| OAUTH-003 | Environment variables containing secrets not marked as "sensitive" | Regex: Scan platform config (Vercel, Railway, Coolify) for env vars matching `*KEY*\|*SECRET*\|*TOKEN*\|*PASSWORD*` without sensitive/encrypted flag. | CRITICAL | 3 |
| OAUTH-004 | OAuth callback URLs using HTTP (not HTTPS) or pointing to untrusted domain | Config scan: Parse OAuth redirect_uri configs; flag non-HTTPS or public-internet domains not in allowlist. | CRITICAL | 1, 9 |
| OAUTH-005 | Missing OAuth app inventory or audit log integration | Policy check: Verify Google Workspace / Entra ID admin console has OAuth app approval gating enabled. | HIGH | 9 |
| OAUTH-006 | Single OAuth token grants access across multiple disparate service boundaries | Trust graph: Identify tokens with cross-domain access (e.g., Google Workspace token also reaching GitHub/Vercel/Linear). Flag for scope compartmentalization. | HIGH | 9 |

**Verification Commands:**

```bash
# Verify OAuth token scopes against least privilege (provider-specific)
curl -s https://api.oauth-provider/v1/permissions/$TOKEN_ID | jq '.scopes'

# Google Workspace: List all authorized third-party OAuth apps
# (requires admin.directory.v1 API access)
gcloud auth application-default print-access-token | \
  xargs -I{} curl -s -H "Authorization: Bearer {}" \
  "https://admin.googleapis.com/admin/directory/v1/tokens/users/$USER_EMAIL"

# Scan for env vars missing "sensitive" flag (Vercel example)
vercel env ls --json | jq '.[] | select(.type != "sensitive") | select(.key | test("KEY|SECRET|TOKEN|PASSWORD"))'

# Find OAuth redirect URIs in codebase
grep -rE "redirect_uri|callback_url|REDIRECT_URL" . --include="*.ts" --include="*.js" --include="*.env*"
```

### 13.2 AI Credential Harvesting via Supply Chain

**Source pattern:** Bitwarden CLI / npm ecosystem supply chain attacks (2025-2026). Malicious packages in build/install steps specifically target AI-related credentials: `ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, `.claude/` directories, SSH keys, and `.env` files.

**Credential patterns to scan for (in code, CI scripts, and `postinstall` hooks):**

| Pattern | Target | Detection |
|---|---|---|
| `ANTHROPIC_API_KEY` / `OPENAI_API_KEY` / `GOOGLE_AI_KEY` | AI service credentials | Regex + entropy analysis |
| `.claude/`, `.cursor/`, `.copilot/` | AI tool config directories | File system access monitoring |
| `~/.ssh/id_rsa`, `~/.ssh/id_ed25519` | SSH private keys | Unexpected file reads in non-SSH code |
| `.env`, `.env.local`, `.env.production` | Environment files | Read access from `postinstall` scripts |
| `AI_GATEWAY_API_KEY`, `VERCEL_TOKEN` | Platform tokens | Hardcoded string detection |

**Supply Chain Scanning Rules:**

| Rule ID | Detection Pattern | Detection Logic (Package/File Analysis) | Severity | Phase |
|---|---|---|---|---|
| SC-001 | Malicious `postinstall`/`preinstall` script | Scan `package.json` for lifecycle scripts containing `curl`, `wget`, `fetch`, `net.request`, or filesystem traversal (`../`, `~/.ssh`, `~/.claude`). | CRITICAL | 2, 3 |
| SC-002 | Dependency version spike / hijack | Compare current dependency versions against known-compromised-version database; flag sudden major version jumps in patch-level packages. | HIGH | 2 |
| SC-003 | Unsigned or unverifiable package | Package lacks Sigstore/cosign signature or SLSA provenance attestation. | MEDIUM | 2, 5 |

**Credential Harvesting Scanning Rules:**

| Rule ID | Detection Pattern | Detection Logic (Credential/Exfiltration Analysis) | Severity | Phase |
|---|---|---|---|---|
| CRED-001 | `postinstall` script reads files outside `node_modules/` | `strace` / syscall monitoring: flag `open()` calls to paths outside package directory during install. | CRITICAL | 2, 3 |
| CRED-002 | Build script spawns network connections to non-registry domains | Network monitoring: capture outbound connections from install scripts; flag any domain not in `{registry.npmjs.org, pypi.org, crates.io}` allowlist. | CRITICAL | 2, 3 |
| CRED-003 | Hardcoded AI API key pattern in source | Regex: `sk-ant-[a-zA-Z0-9_-]{20,}`, `sk-proj-[a-zA-Z0-9_-]{20,}`, `sk-[a-zA-Z0-9]{48,}`, `key-[a-zA-Z0-9]{32,}`. Entropy analysis for 128+ bit strings near `API_KEY` context. | CRITICAL | 3 |
| CRED-004 | `.env` file read by dependency code (not application code) | AST analysis: trace `fs.readFileSync` / `dotenv.config()` calls originating from `node_modules/` to files matching `.env*`. | HIGH | 9 |
| CRED-005 | `process.env` access in dependency for keys matching AI/cloud patterns | AST: scan `node_modules/` for `process.env.ANTHROPIC_*`, `process.env.OPENAI_*`, `process.env.AI_GATEWAY_*`, `process.env.VERCEL_*`. | HIGH | 9 |
| CRED-006 | Outbound HTTP from `postinstall` to non-npm/non-pypi domains | Network + file analysis: Identify HTTP client imports (`axios`, `node-fetch`, `https.request`) in lifecycle scripts with non-registry URLs. | CRITICAL | 2, 3 |
| CRED-007 | AI config directory access from unexpected module | File monitoring: flag reads to `~/.claude/`, `~/.cursor/`, `~/.copilot/`, `~/.ai-tool/` from code paths that aren't the corresponding AI tool itself. | CRITICAL | 3, 9 |
| CRED-008 | Secrets exfiltration via network payload | Runtime: Monitor outbound HTTP request bodies for strings matching API key/token regex patterns (high-entropy + prefix match). | HIGH | 6 |

**Verification Commands:**

```bash
# Find all postinstall/preinstall scripts in dependency tree
find . -name "package.json" -exec grep -lH "postinstall\|preinstall" {} \;

# Audit postinstall scripts for dangerous patterns
find ./node_modules -name "package.json" -exec grep -A3 "postinstall" {} \; \
  | grep -E "curl|wget|fetch|net\.|http\.|~\/|\.\.\/|\.env|\.ssh|\.claude"

# strace a postinstall script (Linux — run in CI sandbox)
strace -f -e trace=open,openat,connect,sendto \
  npm run postinstall --prefix ./node_modules/suspect-package 2>&1 \
  | grep -v "node_modules\|/usr\|/lib"

# Scan for hardcoded AI API keys in codebase
grep -rE "sk-ant-[a-zA-Z0-9_-]{20,}|sk-proj-[a-zA-Z0-9_-]{20,}|ANTHROPIC_API_KEY\s*=\s*['\"]sk-" \
  --include="*.ts" --include="*.js" --include="*.py" --include="*.go" --include="*.env*" .

# Find dependency code accessing process.env for AI keys
grep -rn "process\.env\.\(ANTHROPIC\|OPENAI\|AI_GATEWAY\|VERCEL_TOKEN\)" ./node_modules/ | head -20
```

**Mandatory protections:**
- Run `strace` or equivalent syscall monitoring on `postinstall` scripts in CI.
- Use `--ignore-scripts` for `npm install` in CI; explicitly allowlist trusted postinstall scripts.
- Audit new dependencies for file system access patterns before adoption.
- Store all AI keys in a secrets manager (Vault, AWS SM, Doppler); inject at runtime only.
- Never paste credentials into AI chat interfaces (Lovable breach exposed credentials embedded in chat histories).

### 13.3 Vibe-Coding Platform Security (AI Code Generation Risks)

**Source pattern:** Lovable, Bolt, Replit Agent — AI-generated code frequently contains security vulnerabilities because models are trained on pre-security-era codebases.

**Risks specific to AI-generated code:**

| Risk | Example | Mitigation |
|---|---|---|
| Missing authz checks | AI generates CRUD routes without ownership validation | BOLA scanning rules (Section 12.1) |
| Embedded credentials | User pastes API keys into AI chat; code embeds them | Secret scanning in Phase 3; never paste secrets into chat |
| Weak crypto defaults | AI generates MD5 hashing or ECB mode encryption | SAST rules for deprecated algorithms |
| Missing input validation | AI skips sanitization on user inputs | Injection scanning (Phase 1) |
| Over-permissive CORS | AI generates `Access-Control-Allow-Origin: *` | CORS policy scanning |
| Disabled security features | AI disables RLS, CSP, or CSRF protection for "simplicity" | Configuration audit in Phase 5 |

**MANDATORY for any codebase using AI-generated code:** Every line of AI-generated code MUST pass the same SAST/DAST review pipeline as human-written code. No exceptions. The 9-phase security pipeline (Section 15) applies regardless of code origin.

### 13.4 Attack Vector Classification Schema

All findings from the security pipeline MUST include an `attack_vector_class` field for prioritized routing:

```json
{
  "finding_id": "DEEPSEC-2026-0042",
  "attack_vector_class": "BOLA",
  "severity": "CRITICAL",
  "cwe": "CWE-639",
  "file": "internal/api/projects.go",
  "line": 147,
  "description": "GetProjectMessages returns data without ownership check",
  "remediation": "Add WHERE owner_id = $user_id to query; enable RLS"
}
```

**Defined attack vector classes:**

| Class | Description | Default Priority | Routing |
|---|---|---|---|
| `BOLA` | Broken Object Level Authorization / IDOR | P0 — immediate | Security team + engineering |
| `OAUTH_SCOPE_CREEP` | Over-permissioned OAuth tokens; lateral movement risk | P0 — immediate | Security team + IT ops |
| `CREDENTIAL_HARVESTING` | Secrets targeted via supply chain or build pipeline | P0 — immediate | Security team + DevOps |
| `SUPPLY_CHAIN` | Malicious dependency, tampered build, unsigned artifact | P1 — 24hr | Security team |
| `AUTH_BYPASS` | Authentication or session management failure | P0 — immediate | Security team + engineering |
| `INJECTION` | SQL, XSS, command, template injection | P1 — 24hr | Engineering |
| `CRYPTO_FAILURE` | Weak algorithms, plaintext secrets, poor key mgmt | P1 — 24hr | Engineering |
| `CONFIG_DRIFT` | Misconfiguration, disabled security features | P2 — 72hr | DevOps |
| `LOGIC_FLAW` | Business logic vulnerability, race condition | P1 — 24hr | Engineering |
| `GENERAL_VULN` | Other vulnerability not classified above | P2 — 72hr | Engineering |

---

## 14. Incident Response & Resilience

### 14.1 Incident Response Plan (NIST SP 800-61 Rev 2)
...
### 14.2 Disaster Recovery / Business Continuity
...
### 15.5 CSF 2.0 Organizational Profile
...
## 16. Nine-Phase Security Scanner Pipeline (security-scanner v3)
...
├───────┼────────────────────┼──────────────────┼─────────────────────────────────────┤
│ QUICK │ 1-3                │ Every PR          │ ubuntu-latest          (~2-5 min)  │
│ FULL  │ 1-3 + 5-6          │ Push to main/cron │ ubuntu-latest         (~10-20 min) │
│ DEEP  │ 1-3 + 5-7          │ Schedule/dispatch │ ubuntu-latest         (~30-60 min) │
│ P8    │ 8 (exploit verify)  │ Manual dispatch   │ ubuntu-latest          (opt-in)   │
...
| 7 | Fuzz + Race | go-fuzz, race detector | Memory corruption, data races, panic-on-input |
...
### 16.4 Gate Logic (aggregate.py + gate.py)
...
### 16.5 Phase 8 — Exploit Verification (scope.txt)
...
### 16.6 CI/CD Gate Policy (per-phase enforcement)
...
### 16.7 Phase-to-Rule Mapping (Complete Cross-Reference)

This table maps every scanning rule from Sections 12-13 to the pipeline phase(s) that execute it:

| Phase | Rules Executed | Detection Method | Automated? |
|---|---|---|---|
| **1 — SAST** | BOLA-001, BOLA-002, BOLA-003, BOLA-005, BOLA-006, OAUTH-004 | Regex, AST analysis, SQL schema audit (gosec + custom rules) | ✅ Full |
| **2 — Dependencies** | SC-001, SC-002, SC-003, CRED-001, CRED-002, CRED-006 | Package manifest analysis, version spike detection, strace on install scripts (osv-scanner + custom) | ✅ Full |
| **3 — Secrets** | CRED-003, CRED-007, OAUTH-002, OAUTH-003 | Regex + entropy analysis, filesystem monitoring, env var audit (TruffleHog/gitleaks + custom patterns) | ✅ Full |
| **5 — Container** | SC-003 | Image signature verification, SBOM cross-reference (grype + syft + cosign) | ✅ Full |
| **6 — DAST** | BOLA-003, BOLA-004, BOLA-007, CRED-008 | Runtime HTTP testing: cross-tenant access, response code analysis, payload inspection (nuclei + custom templates) | ✅ Full |
| **7 — Fuzz** | (general) | Panic detection, race conditions, memory corruption (go-fuzz + `-race`) | ✅ Full |
| **8 — Exploit** | (confirms Phase 1-7 findings) | Active exploitation against scoped targets (go-exploit) | ⚠️ Manual trigger |
| **9 — AI Review** | BOLA-001, BOLA-002, BOLA-005, BOLA-006, BOLA-007, OAUTH-001, OAUTH-004, OAUTH-005, OAUTH-006, CRED-004, CRED-005, CRED-007 | Semantic analysis, multi-file taint tracing, business logic reasoning (deepsec) | ⚠️ Manual trigger |

**Rule coverage by attack vector class:**

| Attack Vector Class | Phase 1 | Phase 2 | Phase 3 | Phase 6 | Phase 9 | Total Rules |
|---|---|---|---|---|---|---|
| `BOLA` | ✅ 5 | — | — | ✅ 3 | ✅ 5 | 7 |
| `OAUTH_SCOPE_CREEP` | ✅ 1 | — | ✅ 2 | — | ✅ 4 | 6 |
| `CREDENTIAL_HARVESTING` | — | ✅ 4 | ✅ 2 | ✅ 1 | ✅ 3 | 8 |
| `SUPPLY_CHAIN` | — | ✅ 3 | — | — | — | 3 |

---

## 17. Phase 9 — deepsec AI Deep Review

### 17.1 What is deepsec?
...
### 17.5 INFO.md — Project Context Injection
...
### 17.6 deepsec.config.ts — Configuration Reference
...
### 17.8 Self-Hosted Runner Setup (Ollama)
...
### 17.11 Semantic Anomaly Detectors (2026 Pattern Integration)

These detectors extend deepsec's built-in matchers with rules derived from the 2026 penetration patterns (Section 13). Add these to `deepsec.config.ts` via the `plugins` array or as custom matchers.

| Detector ID | Pattern Name | What It Catches | Maps To |
|---|---|---|---|
| `id_naming_violation` | BOLA/IDOR ownership gap | API handler uses client-supplied resource ID without ownership WHERE clause | Section 13, Lovable BOLA |
| `auth_divergence` | OAuth scope creep | OAuth scope request is broader than the API calls actually exercised | Section 13.1, Vercel/Context.ai |
| `crypto_harvest_divergence` | Credential file access anomaly | Code path reads `~/.ssh/`, `.env`, or AI config dirs from unexpected module | Section 13.2, Supply chain |
| `rls_policy_gap` | Missing/disabled RLS | Multi-tenant table lacks RLS or has `USING (true)` | Section 12.1, BOLA-005/006 |
| `env_var_plaintext` | Unencrypted secrets in env | Environment variable containing key/token/secret pattern not marked sensitive | Section 13.1, OA-006 |
| `postinstall_escape` | Build script sandbox escape | `postinstall` / `preinstall` script accesses paths outside package directory | Section 13.2, CRED-001 |

**deepsec promptAppend addition for 2026 patterns:**
```typescript
promptAppend: [
  // ... existing prompts ...
  "PRIORITY: Check every API endpoint for BOLA — verify user_id == resource.owner_id.",
  "FLAG: OAuth integrations with scopes broader than actual API usage.",
  "FLAG: AI credentials (ANTHROPIC_API_KEY, OPENAI_API_KEY) in code, env, or chat logs.",
  "FLAG: postinstall scripts that read files outside node_modules/.",
  "FLAG: Environment variables containing secrets not marked as sensitive/encrypted.",
].join("\n"),
```

### 17.12 Triangulation Layer Enhancement

When `aggregate.py` processes findings from multiple phases, apply weighted scoring based on the `attack_vector_class` (Section 13.4):

| Condition | Weight Multiplier | Rationale |
|---|---|---|
| Finding confirmed by ≥2 phases (e.g., SAST + deepsec) | 2.0x | Cross-validation increases confidence |
| Finding confirmed by both low-level (regex) and high-level (semantic) detectors | 2.5x | Different detection methods corroborate |
| `attack_vector_class` is `BOLA` or `CREDENTIAL_HARVESTING` | 1.5x base severity | 2026 breach patterns — highest real-world impact |
| Finding is in `priorityPaths` AND has `attack_vector_class` | 1.3x | High-leverage code path with known vector |

---

## 18. OpenClaw / Kilo Code Integration Hooks

### 18.1 Skill Activation Triggers
...
- "BOLA", "IDOR", "OAuth scope", "credential harvesting", "vibe coding security"

### 18.2 SECURITY_AUDIT_MODE
When activated with `SECURITY_AUDIT_MODE`, perform a ten-pass audit:
...
| **8** | **AI/SaaS Attack Vectors (2026)** | **BOLA ownership check audit; OAuth scope inventory; AI credential scan; postinstall analysis** |
| **9** | **Security Pipeline Audit** | **Verify all phases deployed; check scope.txt, INFO.md, gate thresholds** |
| **10** | **deepsec AI Deep Review** | **Trigger Phase 9 with selected provider; review P0/P1/P2 findings with attack_vector_class** |

### 18.3 OSS Security Gating (from GODMYTHOSV7)
...
| `postinstall` script reads `~/.ssh/`, `.env`, or AI config dirs | BLOCK — Credential harvesting (Section 13.2) |
| `postinstall` script opens network connections to non-registry domains | BLOCK — Supply chain exfiltration |
| Hardcoded `ANTHROPIC_API_KEY` / `OPENAI_API_KEY` / `AI_GATEWAY_API_KEY` patterns | BLOCK — AI credential leak |
| OAuth scope requests `https://www.googleapis.com/auth/drive` or broader | WARN — Over-permissioned OAuth (Section 13.1) |
| Missing ownership check on API resource endpoints (BOLA pattern) | BLOCK — Broken Object Level Authorization |
...
- **Lovable BOLA Breach (April 2026)**: https://www.halborn.com/blog/post/lovable-data-leak-bola-vulnerability-and-app-security-risks
- **Vercel/Context.ai OAuth Supply Chain Breach (April 2026)**: https://vercel.com/kb/bulletin/vercel-april-2026-security-incident
- **Vercel Breach Analysis (Trend Micro)**: https://www.trendmicro.com/en_us/research/26/d/vercel-breach-oauth-supply-chain.html
- **Vercel Breach OAuth Analysis (Push Security)**: https://pushsecurity.com/blog/unpacking-the-vercel-breach
- OWASP API Security Top 10 2023: https://owasp.org/API-Security/editions/2023/en/0x11-t10/

---

*Ultimate Security Skill v2.3 — Last updated: 2026-05-18*
*Includes security-scanner v3 (9-phase pipeline) + deepsec Phase 9 AI deep review + 2026 AI/SaaS penetration patterns + four-tier scanning rules with detection logic and verification commands.*
*For use with GODMYTHOSV7+, Kilo Code, OpenClaw, Claude Code, and all AI-assisted development workflows.*

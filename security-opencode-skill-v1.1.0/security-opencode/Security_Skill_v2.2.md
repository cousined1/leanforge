# Ultimate Security Skill v2.2: Defense-Grade SaaS Architect
...
> "Evaluate this code/architecture against the Ultimate Security Skill v2.2. Does it enforce Zero Trust, Least Privilege, FIPS-validated encryption, OWASP Top 10:2025 mitigations, supply chain integrity, SBOM generation, NIST CSF 2.0 Govern-function alignment, the 9-phase security-scanner pipeline with deepsec AI deep review, BOLA ownership validation, OAuth scope minimization, and AI credential protection? If not, rewrite to enforce compliance."
...
| Rule ID | Detection Pattern | Severity |
|---|---|---|
| BOLA-001 | API endpoint returns data without verifying `user_id == resource.owner_id` | CRITICAL |
| BOLA-002 | Database query uses client-supplied ID without WHERE clause on tenant/owner | CRITICAL |
| BOLA-003 | Sequential/predictable resource IDs (integers) exposed in API | HIGH |
| BOLA-004 | API returns 403 (instead of 404) on unauthorized access — enables enumeration | MEDIUM |
| BOLA-005 | Missing Row-Level Security (RLS) policies on multi-tenant tables | CRITICAL |
| BOLA-006 | RLS policy exists but is disabled or has `USING (true)` clause | CRITICAL |
...
| Rule ID | Detection Pattern | Severity |
|---|---|---|
| OAUTH-001 | OAuth scope requests broader than the API calls actually used | HIGH |
| OAUTH-002 | Long-lived OAuth tokens (>90 days) without rotation | HIGH |
| OAUTH-003 | Environment variables containing secrets not marked as "sensitive" | CRITICAL |
| OAUTH-004 | OAuth callback URLs using HTTP (not HTTPS) | CRITICAL |
| OAUTH-005 | Missing OAuth app inventory or audit log integration | HIGH |
...
**Supply Chain Credential Scanning Rules:**

| Rule ID | Detection Pattern | Severity |
|---|---|---|
| CRED-001 | `postinstall` script reads files outside `node_modules/` | CRITICAL |
| CRED-002 | Build script spawns network connections to non-registry domains | CRITICAL |
| CRED-003 | Hardcoded AI API key pattern in source (sk-ant-*, sk-proj-*, etc.) | CRITICAL |
| CRED-004 | `.env` file read by dependency code (not application code) | HIGH |
| CRED-005 | `process.env` access in dependency for keys matching AI/cloud patterns | HIGH |
| CRED-006 | Outbound HTTP from `postinstall` to non-npm/non-pypi domains | CRITICAL |
...
*Ultimate Security Skill v2.2 — Last updated: 2026-05-16*
*Includes security-scanner v3 (9-phase pipeline) + deepsec Phase 9 AI deep review + 2026 AI/SaaS penetration patterns.*
